---
project: project02-language-toolchain-alignment
status: active
planned-release: "0.6.0"
depends-on: ["project01-mvp"]
blocks: []
related: ["project03-language-evolution"]
---
...
> now reorganized into the canonical `project02-language-toolchain-alignment/` layout per
...
the design docs (`project01-mvp/artifacts/design-index.md/`, odm-managed DDs) and `docs/philosophy.md`;
...
0.6.0 commitments), and the odm-managed DDs in `project01-mvp/artifacts/design-index.md/` (DD-36, DD-37,
...
> `project03-language-evolution/BACKLOG.md`** — the single canonical 0.7.0+ candidate
...
| **arc09 · release-0.6.0** | Version bumps, release notes, publish to JSR / npm / crates.io | all above, **incl. arc16 (book)** | **Open — slice01, slice02, and slice03 closed/CDC-verified; slice04 operator-deferred before publication; slice05 `release-candidate-uat-feedback` active** |
...
- [arc15.1-macro-entry-diagnostics](arc15.1-macro-entry-diagnostics/arc-plan.md): preserves the originally standalone macro-resolution diagnostic slice; no release gate added.
- [arc16.1-artifact-homes](arc16.1-artifact-homes/arc-plan.md): preserves the artifact-home and citation-gate slice; chronology tied with arc16.
- [arc16.2-citation-repoint](arc16.2-citation-repoint/arc-plan.md): preserves the subsequent citation cleanup slice.

Decimal identifiers are an operator-approved retrospective exception. Historical slice status is retained; these wrappers add no acceptance claim.
...
> **Historical standalone-slice organization (2026-07-24; superseded by the 2026-09-06 arc wrappers).** Not every unit of 0.6.0 work is an
...
> exist under `project03-language-evolution/` (`01-treeshake-audit` etc.); this tree now
> uses the same convention. Current standalone slices:
>
> - **[`01-macro-entry-diagnostics`](arc15.1-macro-entry-diagnostics/slice01-macro-entry-diagnostics/slice-plan.md)** —
...
> - **[`02-artifact-homes`](arc16.1-artifact-homes/slice01-artifact-homes/slice-plan.md)** — give every
>   durable artifact a tracked home and make "a cited path resolves in git" a
>   `make check` gate (**closed**, 2026-08-08). Origin:
>   `backlog/discoveries.md` was cited by five committed documents while
...
> - **[`03-citation-repoint`](arc16.2-citation-repoint/slice01-citation-repoint/slice-plan.md)** — execute the
...
- **Closed / gated arcs:** arc01, arc02, arc03, arc04, arc05, arc06, arc08,
  arc07, arc10, arc11, arc12, arc13, arc15. arc10's original 2026-07-05 gate
  remains historically true, and the 2026-08-08 slice04 follow-up is closed.
...
- **Open arcs:** arc09 release planning/release cut; slice01
  `release-readiness-runbook`, slice02 `version-bump-release-notes-ci-chore`,
  and slice03 `publish-dry-runs-and-package-audit` are closed/CDC-verified;
  slice03 carries qualified crates sequencing; slice04
  `operator-publication-and-tags` is closed/operator-deferred before
  publication; slice05 `release-candidate-uat-feedback` is open. arc16 is
  closed/CDC-verified.
...
### v1.82 - 2026-09-12 (arc09 slice04 operator-deferred; slice05 active)

The operator declined publication/tag/push for now and directed UAT in other
projects before the full 0.6.0 delivery. slice04 is closed as
operator-deferred before publication; no registry publication, source tag,
branch push, tag push, book tag, or book publication occurred. slice05 is open
to capture release-candidate UAT feedback against source commit `50608c4`.

### v1.81 - 2026-09-12 (arc09 slice03 CDC verified; slice04 active)

CDC reproduced arc09 slice03's package-audit and dry-run evidence against
source commit `50608c4`. The embedded `lykn-lang` package repair is in the
crate tarball, `make check` passed outside the sandbox home-log restriction,
dist metadata reports `0.6.0`, JSR/npm dry-runs passed, and `lykn-lang` crates
dry-run passed. `lykn-cli` and `lykn` remain qualified by normal crates.io
dependency ordering until internal 0.6.0 crates are published. slice04 is
active for operator-approved publication and tags.

### v1.80 - 2026-09-12 (arc09 slice02 CDC verified; slice03 active)

CDC independently reproduced arc09 slice02's release-prep evidence. Source
commit `65ff40f` carries the 0.6.0 version surfaces, draft release notes,
checkout v5 maintenance, and repaired no-bypass `make publish-dry-run`; rebuilt
CLI and generated dist metadata report `0.6.0`; the full local gate passed
outside the sandbox home-log restriction; no publication, release tag, branch
push, or book tag was performed. slice03 remains active for dry-runs and
package audit.
...
CDC reproduced arc16 slice12's final book gates: HTML/EPUB build, generated
edition metadata, whole-book `--fence lisp` 428/0 with 19 skipped blocks,
code-aware link sweep, SUMMARY sweep, stale-caveat sweep, and repo hygiene.
P-20 is done/CDC-verified. arc09 slice01 `release-readiness-runbook` is open
to prepare the detailed 0.6.0 release runbook before version bumps,
publication dry-runs, tags, or post-publish verification.

### v1.77 - 2026-09-12 (arc16 closed by CC; arc09 next)

arc16 slice12 closed the Lykn Book 0.6.0 edition gate in book commit `de0342c`:
final HTML/EPUB build passed, generated HTML/EPUB carry the edition markers,
whole-book `--fence lisp` passed at 428/0 with 19 skipped blocks, link/path and
stale-caveat sweeps were clean, and no new release-blocking finding remained.
P-20 is done from CC's side, CDC verification remains pending, and arc09
`release-0.6.0` is now the current Project02 arc for release planning and the
release cut.

### v1.76 - 2026-09-12 (arc16 slice11 CDC verified)
...
19 skipped blocks. arc16 slice11 is closed/CDC-verified; arc16 slice12 remains
open for the edition close and release-gate pass.

### v1.75 - 2026-09-12 (arc16 slice11 CC-closed; slice12 opened)

arc16 slice11 fixed `D-2609-FOVL`: the JS API now compile-rejects overlapping
multi-clause `func` definitions before emitting dispatch code, matching the CLI
compile path. Source commit `2a0cabf` added the JS overlap check and regression
coverage; book commit `43cfebc` restored `src/part2/chapter8/4-overlap.md`
examples as `lisp,compile-fail`. Verification passed: focused JS regression
7/0, `make check`, focused overlap chapter doctest 4/0, and whole-book
`--fence lisp` 428/0 with 19 skipped blocks. arc16 slice12
`edition-close-and-release-gate` is now open; P-20 remains open until the book
edition closes and arc09 can proceed.

### v1.72 - 2026-09-12 (arc16 slice09 CDC verified)

CDC reproduced the slice09 evidence: the focused touched-chapter `--fence lisp`
gate passed 5/0 with 13 skipped macro-context fragments, `mdbook build -d book`
passed with only the existing mdbook-mermaid warning, the seven-row ledger has
no silent drop, and the book/planning commits include the required trailers.
arc16 slice10 remains open; P-20 remains open until the book edition closes and
arc09 can proceed.

### v1.74 - 2026-09-12 (arc16 slice10 CDC verified)

CDC reproduced the slice10 evidence: the focused touched-chapter `--fence lisp`
gate passed 39/0 with 9 skipped blocks, the whole-book `--fence lisp` gate
passed 425/0 with 22 skipped blocks, and `mdbook build -d book` passed with
only the existing mdbook-mermaid warning. CDC also reproduced `D-2609-FOVL`:
CLI compile rejects overlapping `func` clauses while the JS API emits generated
code. arc16 slice11 remains open; P-20 remains open.

### v1.73 - 2026-09-12 (arc16 slice10 CC-closed; slice11 opened)

arc16 slice10 refreshed the remaining language/compiler book failures in book
commit `6aa379d`. The focused touched-chapter `--fence lisp` gate passed 39/0
with 9 skipped blocks, the whole-book `--fence lisp` gate passed 425/0 with 22
skipped blocks, and `mdbook build -d book` passed. The slice surfaced and routed
`D-2609-FOVL`, so arc16 now inserts slice11 `js-overlap-compile-parity` before
final edition close. P-20 remains open.

### v1.71 - 2026-09-12 (arc16 slice09 CC-closed; slice10 opened)

arc16 slice09 refreshed the Lykn Book toolchain/testing/project-structure
chapter set in book commit `03b3818`. The focused touched-chapter `--fence lisp`
gate passed 5/0 with 13 skipped macro-context fragments, `mdbook build -d book`
passed, and no new implementation defect was routed. arc16 slice10
`language-surface-chapters` is now open; P-20 remains open until the book edition
closes and arc09 can proceed.

### Planning migration — 2026-09-06
...
CDC verified arc16 slice06 against the lang, book, and writers-guide repos.
...
check-cited-paths`, and `git diff --check`. Book probes reproduced the landed
gate: `--fence lisp` generated 176 files from 444 blocks with 417 passing / 27
failing examples; mixed `--fence lisp --fence lykn` generated 177 files from
447 blocks with 420 passing / 27 failing examples. P-20 remains open: slice07
`current-book-drift-refresh` is next, and the book edition itself is not
drafted/closed.

### v1.70 - 2026-09-12 (arc16 slice08 CDC verified)

CDC reproduced the slice08 source and book gates: focused `dd-50.6_test.lykn`
passed 10/0, `cargo fmt --check`, `cargo test -p lykn-cli`, full Deno tests,
`make test-docs`, and `make check-cited-paths` passed, and the affected book
subset passed 3/0. arc16 slice08 is now closed/CDC-verified; slice09 remains
the active chapter slice.
...
for `D-2607-R4NW`. Default doctests still extract only `lykn` fences; the book
command run from `/Users/oubiwann/lab/cnbb/lykn` now reaches the 444 `lisp`
...
Opened arc16 slice06 `book-fence-reachability`. The open set scopes the
`D-2607-R4NW` implementation route for a repeatable `--fence` option on
`lykn test --docs`, default-preserving doctest behavior, opt-in book `lisp`
fence reachability, sibling instruction refresh, Discovery Register
disposition, and first book-level extracted/failing counts. P-20 remains open
and book-facing chapter work is still blocked until the gate lands.

### v1.66 - 2026-08-20 (arc16 slice05 CDC verified)

CDC verified arc16 slice05 against the lang, book, and writers-guide repos.
The standing instruction bootstrap now has independent evidence for stale-path
removal, AGENTS/CLAUDE symlink preservation, current 0.6.0 language guidance,
negative treatment of the absent universal book-test gate, sibling repo commits,
and green lang docs/cited-path gates. P-20 remains open; slice06
`book-fence-reachability` remains next.

### v1.65 - 2026-08-20 (arc16 slice05 closed by CC; CDC pending)

arc16 slice05 closed the standing instruction bootstrap from CC's side. The book
repo and writers-guide repo now point authors at the active 0.6.x lang worktree,
preserve `AGENTS.md` canonical / `CLAUDE.md` symlink compatibility, remove stale
old-repo path guidance, stop teaching raw `deno test test/book/` as a current
universal gate, teach `(exports ...)`, grouped sequential `bind`, and `cond`,
and route book-discovered language/tooling/DevX defects back through the lang
Discovery Register. P-20 remains open because the book is not drafted/closed;
slice06 `book-fence-reachability` remained next and book-facing chapter work was
still blocked on that gate.

### v1.64 - 2026-08-20 (arc16 slice04 opened)

Opened arc16 slice04 `language-surface-runway` with the canonical open set. It
routes `D-2608-XPRT`, `D-2608-LBND`, and `D-2608-COND` into implementation or
explicit deferral before book prose. arc16's plan is now a 10-slice plan after
pushing the provisional book-facing rows back behind the implementation-first
language work.

### v1.63 - 2026-08-20 (arc16 slice03 CDC-verified)

arc16 slice03 is closed/CDC-verified. The first CLI/scaffold/package runway
cluster is no longer a book blocker: fresh scaffolds include `bin/lykn`,
scaffolded Lykn tests run through `testing`, nested package sources build
recursively, source-file `lykn run` handles relative package imports through
build output, and the 0.6.0 source-ownership floor is explicit. P-20 remains
open because the book is not drafted/closed and the language-surface decisions
`D-2608-XPRT`, `D-2608-LBND`, and `D-2608-COND` still need implementation or
explicit deferral before prose normalizes examples.

### v1.62 - 2026-08-08 (arc16 slice03 opened)

arc16 slice03 is open as the CLI/scaffold/package runway implementation slice.
It scopes `D-2608-BINW`, `D-2608-TDSL`, `D-2608-BREC`, `D-2608-RIMP`, and the
0.6.0 source-ownership floor for `D-2608-SOWN`; it leaves exports, grouped
locals, and flatter branching for a later language-surface slice.

### v1.61 — 2026-08-08 (arc16 slice02 closed/CDC-verified)

arc16 slice02 closed at `35f8fcc` and CDC reproduced the scratch project gates.
The pass confirmed that implementation must continue before book prose: fresh
projects still need routing for local binary/scaffold expectations
(`D-2608-BINW`), scaffolded testing macros (`D-2608-TDSL`), nested package build
recursion (`D-2608-BREC`), and source-run relative imports (`D-2608-RIMP`),
alongside the earlier source ownership and language-surface decisions.

### v1.60 — 2026-08-08 (arc16 implementation-first runway)

The operator clarified after arc16 slice01 that all implementation work must
land before the book starts, and that more from-scratch Lykn project dogfood
iterations with CC are expected. P-20 now treats arc16 as an
implementation-first book gate: slice02 opens the dogfood implementation runway,
book/writers-guide/chapter slices are blocked behind accepted implementation
routes, and arc16 A-9 will verify that prose did not mask unsettled language.

### v1.59 — 2026-08-08 (arc16 slice01 closed/CDC-verified)

arc16 slice01 closed at `00b3338`, with CDC verification recorded under
[`slice01-pre-book-decision-gate/cdc-verification.md`](arc16-book-0.6.0-edition/slice01-pre-book-decision-gate/cdc-verification.md).
P-20 remains open because the book edition is not drafted/closed, but the
pre-book decision packet is no longer an open slice. D-3 is resolved to tracked
lang planning homes; D-1 / `D-2607-R4NW` has a fence-first recommendation; and
the export, grouped-binding, flatter-branching, and source-ownership findings
now require operator decisions and explicit implementation/docs routes before
the dependent book slices.

### v1.58 — 2026-08-08 (arc16 planned; slice01 opened)

arc16 now has its canonical [`arc-plan.md`](arc16-book-0.6.0-edition/arc-plan.md)
and an opened slice01 decision gate under
[`slice01-pre-book-decision-gate/`](arc16-book-0.6.0-edition/slice01-pre-book-decision-gate).
The old 6-8 "iteration" book thread has been translated into arc/slice shape:
slice01 re-grounds D-1...D-5, `D-2607-R4NW`, and the four dogfood decisions
before any compiler/scaffold/book prose work proceeds. P-20 remains open and
continues to gate arc09, but the missing arc-plan prerequisite is now closed.

### v1.57 — 2026-08-08 (source-ownership finding added)

The operator clarified that user projects may contain arbitrary user-authored
non-Lykn files, but Lykn-owned generated/config/build manifests should not be
placed in package source trees as if they were author-owned source. Recorded as
dogfood F-10 and permanent discovery `D-2608-SOWN` (`held-for-design`). arc16
planning must settle where generated manifests and package metadata belong
before teaching project structure, build, or publishing examples.

### v1.56 — 2026-08-08 (export-surface finding expanded with `mod.lykn` duplication)

The operator noticed the dogfood package also declares exports in `mod.lykn`.
Expanded `D-2608-XPRT` and dogfood F-7 from "inline export wrappers hide the
public API" to the fuller question: are implementation-module exports and
entrypoint re-export lists both required, accidental duplication, or distinct
layers? arc16 planning must settle export ownership before teaching the package
module shape.

### v1.55 — 2026-08-08 (flatter validation-branching finding added)

The operator also flagged the nested validation `?` ladder from CC's dogfood
module as a readability smell: repeated nesting of the same conditional form
indicates the surface likely needs a flatter branch construct for ordered
validation. Recorded as dogfood F-9 and permanent discovery `D-2608-COND`
(`held-for-design`). arc16 planning must settle whether Lykn gets `cond`,
guard-style validation, or another branch surface before the book teaches
validation/control-flow examples.

### v1.54 — 2026-08-08 (grouped-local-binding dogfood finding added)

The operator also rejected repeated sibling local `(bind name value)` forms as the
only durable way to express several derived locals. Recorded as dogfood F-8 and
permanent discovery `D-2608-LBND` (`held-for-design`). arc16 planning must settle
the grouped local-binding syntax and semantics before drafting expression,
validation, and normalization examples; any compiler/docs work must be routed
before arc16 closes rather than teaching verbose repeated binds as the permanent
idiom.

### v1.53 — 2026-08-08 (export-surface dogfood finding added)

The operator rejected inline `(export (func ...))` as the durable Lykn teaching
shape after CC's external-library dogfood report showed realistic modules hiding
their public API inside definition wrappers. Recorded as dogfood F-7 and
permanent discovery `D-2608-XPRT` (`held-for-design`). arc16 planning must settle
the top-of-module export syntax before drafting module/API chapters, and any
compiler/docs work must be routed before arc16 closes rather than normalized in
the book by inertia.

### v1.52 — 2026-08-08 (arc10 slice04 closed; P-22 done)

arc10 slice04 fixed `D-2608-W2HF`: no-else `if` in expression position now
fails `lykn check`/`compile` before invalid JavaScript can be emitted, while
statement-position no-else `if` and expression-position `if` with an else branch
remain valid. P-22 is done and arc10 is closed again. The remaining release
path is now arc16, then arc09.
...
Deno examples only where they teach Deno itself. The live no-else `if` compiler
defect it routed as `D-2608-W2HF` is now fixed in arc10/P-22.

The remaining release path is arc16, then arc09.

### v1.50 — 2026-08-08 (arc07 slice04 CDC-verified)

arc07 slice04 (`ae31c75`) is CDC-verified: guides 12-01, 12-02, and 12-03 now
route normal project workflows through lykn wrappers while retaining direct
Deno examples only where they teach Deno permission/runtime/API/assertion/task
mechanics. CDC also repaired adjacent guide 10 and guide 15 supporting-reference
drift so the docs surface consistently points generated output at
`target/lykn/`, not repo-root `dist/`.

P-13 remains open until arc07 closes its composition rows A-3/A-4/A-5.

### v1.49 — 2026-08-08 (arc07 slice04 opened)

Opened arc07 slice04, **deno-workflow-reconciliation**, with the full open set.
P-13 remains open; the active docs work is now reconciling guides 12-01,
12-02, and 12-03 so normal project workflows use lykn wrappers while Deno
runtime/API/task-runner teaching remains intact where intentional.
...
- **The Discovery Register moved to `docs/backlog/discoveries.md`** — *the path
  ...
  `backlog/README.md` carries the row format, the section scheme, and the
...
`backlog/discoveries.md`, including a candidate **systemic** entry — "no
...
**Process findings bubbled up** (both in `backlog/discoveries.md`): the
...
`project03-language-evolution/BACKLOG.md` (§A big-rock arcs incl. **A6 fully-typed
...
separate `project03-language-evolution/` tree). Grounding slice03 against the actual
...
- Adopted the canonical `project02-language-toolchain-alignment/` layout (operator-confirmed
