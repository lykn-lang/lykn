...
> slice01 through slice12 are closed/CDC-verified. slice12 completed the final
> edition close in book commit `de0342c`, with HTML/EPUB build, whole-book
> `--fence lisp` doctests, link/path/version sweep, and final repository hygiene
> reproduced by CDC. arc16 hands Project02 back to arc09 for release planning
> and the 0.6.0 release cut.
...
### v1.28 - 2026-09-12 (slice12 and arc16 CDC verified)

CDC reproduced slice12's final book gates: `mdbook build -d book`, generated
HTML/EPUB edition metadata, whole-book `--fence lisp` 428/0 with 19 skipped
blocks, code-aware link sweep, SUMMARY sweep, stale-caveat sweep, and repo
hygiene. arc16 is closed/CDC-verified and arc09 slice01
`release-readiness-runbook` is open.

### v1.27 - 2026-09-12 (arc16 closed by CC; CDC pending)

slice12 closed the edition gate in book commit `de0342c`: final HTML/EPUB build
passed, EPUB contents and edition markers were spot-checked, whole-book
`--fence lisp` passed at 428/0 with 19 skipped blocks, link/path/stale-caveat
sweeps were clean, and no new release-blocking finding remained. arc16 is closed
from CC's side and hands Project02 back to arc09 release planning.
...
slice11 fixed `D-2609-FOVL`: the JS API now rejects overlapping multi-clause
`func` definitions before emitting first-match dispatch code. Source commit
`2a0cabf` added the overlap check and JS regression coverage; book commit
`43cfebc` restored the overlap chapter examples as `lisp,compile-fail`.
Verification passed: focused JS regression 7/0, `make check`, focused overlap
chapter doctest 4/0, and whole-book `--fence lisp` 428/0 with 19 skipped
blocks. slice12 `edition-close-and-release-gate` is now open.
...
slice10 refreshed the remaining language/compiler chapter failures in book commit
`6aa379d`. The focused touched-chapter book gate passed 39/0 with 9 skipped
blocks, the whole-book `--fence lisp` gate passed 425/0 with 22 skipped blocks,
and `mdbook build -d book` passed with only the existing mdbook-mermaid warning.
The pass surfaced `D-2609-FOVL`: the JS API doctest path emits overlapping
multi-clause `func` code instead of compile-rejecting it like the CLI compile
path. Inserted slice11 `js-overlap-compile-parity`; moved edition close to
slice12. Which-child-surfaced: slice10.

### v1.22 - 2026-09-12 (slice09 CDC verified)

CDC reproduced the slice09 book evidence: the focused touched-chapter
`--fence lisp` gate generated 7 test files from 5 runnable blocks with 13
skipped macro-context fragments and passed 5/0; `mdbook build -d book` passed
with only the existing mdbook-mermaid version warning. The book and planning
commits include the required trailers, the seven-row ledger has no silent drop,
and slice10 remains open for the language-surface chapter pass.
