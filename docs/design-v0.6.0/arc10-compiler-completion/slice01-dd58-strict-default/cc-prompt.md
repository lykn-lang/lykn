...
`.lykn` compilation, then migrate the repo. This makes surface lykn actually
prevent the kernel-form leaks the design and `philosophy.md` claim it prevents.
**It lands; it's breaking. Repo-only** (downstream/mycelium is a filed follow-up).
...
3. **F-3 — migration audit (repo-only).** Enumerate every bare kernel-only form
   in surface `.lykn` + guide doctests (repo grep: ~9 `var`, ~57 `const`, ~35
   `===`/`&&`/`function`; hot spots: `docs/guides/{00,01,09}`, `test/`,
   `examples/`). Classify each: **real surface code** → convert to the surface
   form (`bind`/`=`/`and`/`fn`); **documentation / anti-pattern examples** (guide
   09's "bad" examples, guide 00's kernel-form reference) → use the `kernel:`
   escape *or* mark the fence **`compile-fail`** (as W-4d did for ICU); **`.lyk`
   kernel tests** → leave alone.
4. **F-4 — apply the migration** so the whole repo compiles under strict. Repo-only.
5. **F-5 — the A-3 demo.** Confirm each of the 10 leak snippets from your report
   errors in a `.lykn` file, and that `(kernel:var …)` etc. compiles.
...
The guide-09 "eliminated" examples are now `compile-fail`/`kernel:`-escaped (F-8).
