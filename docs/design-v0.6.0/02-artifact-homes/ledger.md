...
| L-5 | `AGENTS.md` in **all three** repos records the artifact layout, the `workbench/`-is-scratch rule, and the cited-path rule; book + writers-guide point at the lang planning home | read all three | serious | **open — CDC verification failed** | 2026-08-08 CDC pass found the content only in untracked sibling-repo `CLAUDE.md` files, not tracked `AGENTS.md` files. The lang repo `AGENTS.md` is tracked and correct. | reproduced (CDC, failed) |
| L-6 | `fences.lykn` has a tracked home in the book repo | `git status` in `~/lab/cnbb/lykn` | polish | **open — CDC verification failed** | 2026-08-08 CDC pass found the book audit tool present on disk but untracked in the book repo. | reproduced (CDC, failed) |
