...
| **arc16 · book-0.6.0-edition** | The Lykn Book's 0.6.0 edition drafted to completion — the full-surface review that catches language and DevX defects **before** the release, not in point releases after it | arc01–08, arc10–13, arc15 (describes shipped behaviour); pairs with arc07 (guides) | **Open — arc-plan WRITTEN 2026-08-08; slice01 `pre-book-decision-gate` OPEN.** *(was: "planning home created, arc-plan still to be written".)* The confirmation-protocol decision (PROJECT-MANAGEMENT Part VI) was taken with the operator on 2026-07-25: **split by design** — the *plan* lives here at [`arc16-book-0.6.0-edition/`](./arc16-book-0.6.0-edition/) because this project's arc09 gates on it; the *content* stays in `~/lab/cnbb/lykn`, whose `AGENTS.md` points back here (as does `~/lab/cnbb/lykn-writers-guide`'s). The previously-untracked material is now tracked under `arc16-book-0.6.0-edition/design/`: the kickoff thread, the drift inventory, the fence-wiring spec, and the dogfooding friction log. The current first slice is a recon/decision gate: re-ground D-1...D-5, `D-2607-R4NW`, `D-2608-XPRT`, `D-2608-LBND`, `D-2608-COND`, and `D-2608-SOWN`; classify each as book/docs-only, 0.6.0 implementation, or 0.7.0+ deferral; and update the arc-plan before any chapter or compiler/scaffold work proceeds. |
...
| P-20 | **arc16 (Lykn Book 0.6.0 edition) drafted and closed** — the book's 0.6.0 edition is complete, and every language/DevX defect it surfaced is either fixed in 0.6.0 or routed with a named home | ptr: arc16 closing-report + the defect-routing table | **serious** | operator decision 2026-07-24 (release-gating call) | **open — CLI/scaffold/package runway active before book prose** | [`arc16-book-0.6.0-edition/arc-plan.md`](./arc16-book-0.6.0-edition/arc-plan.md) opens the arc ledger and a 9-slice implementation-first plan; [`slice01-pre-book-decision-gate/closing-report.md`](./arc16-book-0.6.0-edition/slice01-pre-book-decision-gate/closing-report.md) and [`cdc-verification.md`](./arc16-book-0.6.0-edition/slice01-pre-book-decision-gate/cdc-verification.md) close A-1; [`slice02-dogfood-implementation-runway/closing-report.md`](./arc16-book-0.6.0-edition/slice02-dogfood-implementation-runway/closing-report.md) and [`cdc-verification.md`](./arc16-book-0.6.0-edition/slice02-dogfood-implementation-runway/cdc-verification.md) close the first dogfood/routing pass; [`slice03-cli-scaffold-package-runway/`](./arc16-book-0.6.0-edition/slice03-cli-scaffold-package-runway/) opens implementation work for `D-2608-BINW`, `D-2608-TDSL`, `D-2608-BREC`, `D-2608-RIMP`, and the 0.6.0 floor for `D-2608-SOWN`; D-3 is resolved to tracked lang homes; D-1 / `D-2607-R4NW` has a fence-first recommendation; dogfood F-7 / `D-2608-XPRT`, F-8 / `D-2608-LBND`, and F-9 / `D-2608-COND` remain explicit language-surface decisions before book examples harden | **Gates arc09.** Prerequisite (1) — *the plan gets a git home* — is met. Prerequisite (2) — *the decision packet exists* — is met. Prerequisite (3) is now implementation routing: accepted 0.6.0 language, CLI, scaffold, and package work must close or defer before book/writers-guide/chapter slices normalize the surface. |
