...
| **arc07 · docs** | Guide/SKILL alignment with 0.6.0; clear guide drift; land discoverability additions | arc01–06, arc08, arc15 (describes shipped behaviour) | **Active** — slice01 doctest drift closed; slice02 current-drift recon closed/CDC-verified; next split is slice03 build/dist/publish docs, slice04 Deno workflow reconciliation, plus compiler follow-up `D-2608-W2HF` outside the docs-only stream |
...
- **Open standalone work:** none. `02-artifact-homes` and
  ...
  green at HEAD.
- **Open arcs:** arc07 (active: slice02 recon closed; slice03
  build/dist/publish guide refresh next), arc16 (book arc-plan still to write).
...
| P-13 | docs/guides + SKILL aligned with shipped 0.6.0 (no unreconciled guide drift) | arc07 drift-audit demo; **`make test-docs` green** | correctness | project-plan | open | **doctests now green (476/0)**; arc07 slice01 closed the red-CI item; slice02 current-drift recon closed/CDC-verified and split the remaining work | next docs work: build/dist/publish refresh, Deno workflow reconciliation; compiler defect `D-2608-W2HF` routed so docs do not paper over invalid-JS emission |
