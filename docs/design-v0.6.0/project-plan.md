...
| P-20 | **arc16 (Lykn Book 0.6.0 edition) drafted and closed** — the book's 0.6.0 edition is complete, and every language/DevX defect it surfaced is either fixed in 0.6.0 or routed with a named home | ptr: arc16 closing-report + the defect-routing table | **serious** | operator decision 2026-07-24 (release-gating call) | **open — arc-plan written; slice01 decision gate open** | [`arc16-book-0.6.0-edition/arc-plan.md`](./arc16-book-0.6.0-edition/arc-plan.md) opens the arc ledger and an 8-slice plan; slice01's open set is tracked under [`slice01-pre-book-decision-gate/`](./arc16-book-0.6.0-edition/slice01-pre-book-decision-gate/); design material remains under `arc16-book-0.6.0-edition/design/`; dogfood F-7 / `D-2608-XPRT`, F-8 / `D-2608-LBND`, F-9 / `D-2608-COND`, and F-10 / `D-2608-SOWN` are now explicit slice01 decision gates before book examples harden | **Gates arc09.** Prerequisite (1) — *the plan gets a git home* — is met. Prerequisite (2), the operator's D-1…D-5 decisions plus the new export, local-binding, validation-branching, source-ownership, and fence/test decisions, is now concentrated in slice01. Later slices remain intentionally provisional until the decision gate closes. |
