...
### Q3 — `AGENTS.md` in the book repo: **DECIDED — Option B**

Add a thin `AGENTS.md` to the book repo (`~/lab/cnbb/lykn/`) that
...
(separate from the book repo for cross-book reuse); AGENTS.md
preserves CC tool ecosystem auto-discovery.

`new-ch-prompt.md` stays in the writer's-guide repo as the
chapter-bootstrap template, referenced from `AGENTS.md`.
...
| B0-H | `~/lab/cnbb/lykn/AGENTS.md` | **Missing.** No `AGENTS.md` exists in the book repo. CC's tool ecosystem discovers project conventions via `AGENTS.md`. Authoring guide lives in a separate repo (`lykn-writers-guide`). | Add thin `AGENTS.md` to book repo per decided Q3 (Option B). Single doc, points at writer's-guide canonical paths, lists the standing voice rule, the per-edition versioning model (see Q1 decisions), and the test-gate rule (see B0-G). | open |
...
1. **B0-H first** — add the book repo's `AGENTS.md` so CC sessions have local conventions discovery before any other work.
...
| B3-R | `src/part6/chapter30/`, `src/part6/chapter31/` | Lykn CLI safety gates (`AGENTS.md` "Lykn CLI safety gates" section in lang). Book should reflect the "no auto-pass `--allow-dirty` / `--force` / `--no-verify`" position. M13 lands the publish dirty-check; book documents the gate when M13 closes (Bucket 2, B2-E). | Iter 4: confirm Ch 30/31 don't currently *recommend* bypass flags. Iter 5 (B2-E): document the M13 gate. | open |
