...
| F-2 | Current repo facts verified for lang, book, and writers-guide | closing report records branch/status for lang plus filesystem/git facts for sibling `AGENTS.md`, `CLAUDE.md`, `test/book/`, `book.toml`, and `src/SUMMARY.md` | serious | slice-doc | open | | use evidence, not May inventory claims |
...
| F-4 | `D-2607-R4NW` / book fence reachability strategy reconciled with B0-G | closing report compares `lykn test --docs --fence lisp`, `test/book/`, and any hybrid option; recommends the next implementation route | serious | fence spec + inventory | open | | do not assume old `test/book/` plan still wins |
