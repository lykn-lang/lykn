...
| F-5 docs | **Reproduced**: `test/CONVENTIONS.md` (canonical command with the *why* of each part; location-independence MUSTs; naming split) — the home rationale (repo-suite vs user-docs) is right; `CLAUDE.md` carries the caveat. Lint candidates named in the bubble-up → instantiated in arc05's plan this pass. | reproduced (read) |
