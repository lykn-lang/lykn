...
| **slice02 · mycelium re-audit** | **Recon-only** (CDC did the static pre-audit — mycelium is Cowork-connected now; the **runtime** pass is CC-run on the host): re-point mycelium at current lykn (it pins `@0.5.2`) + smoke-test against current `release/0.6.x`; disposition **each** of the 14 `mycelium-bootstrap-issues.md` issues as **fixed / partial / open** (with the 0.6.0 arc that fixed it), plus any **new** friction; produce the authoritative open-blocker inventory that scopes slices 03/04 and routes any compiler bugs. No fixes — the ground-truth pass. | **Open — scoped** (open set written 2026-07-21) |
| **slice03 · `lykn add`** | The ergonomic dependency-addition command (the DD-51 replacement) — **DD-63 first** (design: what `lykn add <specifier>` writes to `project.json` / the import-map, jsr:/npm:/workspace specifiers, version pinning, Deno-cache population, interaction with per-package `deno.json`). Sized after the audit. | Planned (post-audit; DD-63 pending) |
| **slice04 · external-project path + resolution hardening** | `lykn new` producing a *publishable, testable-outside-the-monorepo* project (the audit's still-open scaffold issues — lang/testing imports to published packages, license, gitignore/JSR); the remaining cross-project resolution blockers. Sized after the audit. | Planned (post-audit) |
...
| A-2 | slice02 (mycelium re-audit) closed | ptr: slice02 cdc-verification | serious | arc-plan | open | | the ground-truth inventory |
| A-3 | slice03 (`lykn add`) closed | ptr: slice03 cdc-verification | serious | arc-plan | open | | DD-63-backed; sized post-audit |
| A-4 | slice04 (external-project path) closed | ptr: slice04 cdc-verification | serious | arc-plan | open | | sized post-audit |
| A-5 | **`lykn add <specifier>` adds a dependency to a project and it resolves** — end-to-end on a fixture project | run `lykn add`; the added dep imports + compiles | serious | arc-plan / DoD (P-6) | open | | reproduce at arc scale on host |
| A-6 | **mycelium builds, tests, and publish-dry-runs as a downstream of current lykn from outside the monorepo** (the composition demo) | host: scaffold/consume lykn from mycelium → `lykn test` + `lykn publish --dry` green | serious | arc-plan | open | | **reproduce at arc scale on host** (mycelium is host-only) |
