...
| [arc05 · lykn-source-linter](./arc05-lykn-source-linter/arc-plan.md) | `lykn lint` over Lykn source | **RESUMED** — 15 rules live; **slice03 scoped 2026-07-21** (open set written) as a 1→2 split: slice03 = resolution-consumer + shadowing + ID-42 re-answer (**closed arc13 A-6**: no rule) + dogfood; **slice04** = suppression + `make lint` + guide-09/15 + SKILL + P-11 → arc close. Next work: **implement slice03** (CC) |
