...
| [arc13 · expander-coherence](./arc13-expander-coherence/arc-plan.md) | Lexical bindings shadow macros on both backends; reserved words rejected; conformance corpus | **Open — 5 slices closed** (binding layer complete by construction; D2 everywhere; coverage test standing); **slices 06–09 closed — the Rust side is done** (DD-60 D1 holds @ `dc37ae9`; DD-61 §A6 fully landed: visibility + static check + corpus, privacy flip @ `4c12301`); **slice10 · js-resolution scoped, ready for CC** → then corpus + close; blocks arc05 slice03 |
