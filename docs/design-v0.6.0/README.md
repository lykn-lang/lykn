...
| [arc05 · lykn-source-linter](./arc05-lykn-source-linter/arc-plan.md) | `lykn lint` over Lykn source | **PAUSED at 2/3** — 15 rules live; blocked on arc13 (slice02's recon exposed the expander divergence) |
| [arc13 · expander-coherence](./arc13-expander-coherence/arc-plan.md) | Lexical bindings shadow macros on both backends; reserved words rejected; conformance corpus | **Open — 5 slices closed** (binding layer complete by construction; D2 everywhere; coverage test standing); **slices 06–10 closed — DD-60 D1 holds on BOTH backends** (Rust @ `dc37ae9`, JS @ `c19a1fb`; divergence 601→56, all explained; DD-61 §A6 fully landed both sides); **slice11 · conformance-corpus + dispositions scoped (the last slice)**, then the arc close-set; blocks arc05 slice03 |
