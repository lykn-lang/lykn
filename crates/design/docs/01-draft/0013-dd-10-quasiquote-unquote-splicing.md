...
using `append` and `list` over internal AST nodes.
...
They produce s-expression AST nodes (lists, symbols, numbers, strings)
...
**Decision**: Quasiquote expansion uses Bawden's `append`/`list` algorithm
(Bawden 1999, "Quasiquotation in Lisp", PEPM). The expansion operates on
internal AST nodes (the same symbol, list, number, and string structures
the reader produces). The macro environment API functions (`list`, `sym`,
...
|---|---|---|
| Literal `x` | `(list (quote x))` | Quoted, wrapped in single-element list |
| Unquote `,x` | `(list x)` | Evaluated, wrapped in single-element list |
| Splice `,@x` | `x` | Evaluated, bare — must be a list |

The quasiquoted list becomes `(append <arg1> <arg2> ... <argN>)`.

**Atom expansion** — a quasiquoted atom (not a list) is simply quoted:
...
;; →
(list (quote if) (quote true) (list (quote console:log) (quote "yes")))
...
(append
  (list (quote if))
  (list test)
  (list (list (quote console:log) (quote "yes"))))
...
quasiquote correctly, and uses only `append` and `list` (never `cons`,
which lykn has no use for). The uniform `append`/`list` structure is
what makes splicing work — splice is just a different wrapping of the
same pattern.
...
`(if true (console:log "yes"))
;; Optimized: return the literal AST list node

;; No splices — can use list instead of append
`(if ,test ,body)
;; Instead of: (append (list (quote if)) (list test) (list body))
;; Optimized:  (list (quote if) test body)
```
...
**Why rejected**: lykn has no `cons` or dotted pairs. The `append`/`list`
formulation from Bawden handles all cases and maps naturally to list/array
operations. No reason to introduce cons cells.
