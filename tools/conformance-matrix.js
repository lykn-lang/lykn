...
const SURFACE_FORMS = [
  "fn", "func", "genfn", "genfunc", "lambda", "and", "or", "not", "bind",
  "cell", "express", "obj", "assoc", "dissoc", "conj", "match", "type",
  "macro",
];
// Kernel form heads (compiler.js `const macros`) that are identifier-shaped and
// not already covered above.
const KERNEL_HEADS = [
  "array", "assign", "async", "await", "block", "const", "let", "var",
  "function", "get", "if", "import", "new", "return", "yield", "do",
];
// JS reserved words (the ID-44 genus — binding any of these emits invalid JS).
const RESERVED_WORDS = [
  "if", "while", "for", "return", "class", "switch", "try", "catch",
  "delete", "typeof", "in", "void", "this", "super", "export", "default",
  "instanceof", "case", "finally", "debugger",
];
...
function isLegalIdent(name) {
  ...
  try {
    ...
    const p = new Deno.Command("deno", { args: ["check", tmp], stdout: "null", stderr: "null" })
      .outputSync();
...
const BIND_ORDER = ["func-param", "bind", "destructuring", "loop", "class-param"];
...
function stripNoise(out, name) {
  return out
    ...
    .filter((l) => {
      const t = l.trim();
      if (!t) return false;
      if (t.includes(": warning:") || t.startsWith("suggestion:") ||
          t.startsWith("• ") || t.includes("warning:")) return false;
      // per-arg runtime type guard mentions the name; not a "use".
      if (t.includes(`typeof ${name} !==`) || t.includes(`arg '${name}'`)) return false;
      return true;
    })
    .join("\n");
}
...
function classify(name, res, refPos) {
  ...
  if (!isLegalIdent(name)) return { cls: "invalid-output", note: firstLine(body) };
...
L.push("");
L.push("Generated by `tools/conformance-matrix.js` — deterministic, re-runnable.");
L.push("Legend: `✓bind`=calls the binding (correct shadowing) · `✗macro`=macro");
L.push("fires instead · `✗throw`=compile error · `✗inval`=invalid JS at rc=0.");
L.push("");
const total = rows.length;
const divergent = rows.filter((r) => !r.agree).length;
L.push(`**${total} cells** · **${divergent} where the backends disagree** ` +
...
for (const [cls, names] of NAME_CLASSES) {
  ...
  for (const name of names) {
    ...
    for (const bpos of BIND_ORDER) {
      ...
      const first = rs[0];
      cells.push(`${SYM[first.rust.cls]}/${SYM[first.js.cls]}` +
        (verdicts.size > 1 ? "∗" : ""));
    }
...
const nonUniformNames = [...new Set(rows
  .filter((r) => {
...
if (nonUniformNames.length) {
  ...
  for (const r of rows) {
    ...
    L.push(`| ${r.name} | ${r.bpos} | ${r.rpos} | ${r.rust.cls} | ${r.js.cls} |`);
  }
  L.push("");
}
